...
const AdminPanel: React.FC<AdminPanelProps> = ({ onBack }) => {
  ...
  const handleConfigureDefaults = (reportName: string) => {
    setEditingDefaultsReport(reportName);
  };

  return (
    <div className="max-w-6xl mx-auto animate-fade-in pb-20">
      {editingDefaultsReport && (
        <DefaultMappingModal
          reportName={editingDefaultsReport}
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {(config[cat.key] || []).map((item) => (
                    <div key={item} className="flex items-center gap-1 bg-slate-100 text-slate-700 px-3 py-1 rounded-full text-sm border border-slate-200 group hover:bg-slate-50 transition-colors">
                      ...
                      {cat.key === 'reportNames' && (
                        <button
                          onClick={() => handleConfigureDefaults(item)}
                          className="w-4 h-4 flex items-center justify-center rounded-full text-slate-400 hover:text-blue-600 transition-colors ml-1"
